    ...
    api
        ...
        .then(function (response) {
            if (response.data.code == 0) {
                ...
                let scope = JSON.parse(data.value.scope)
                for (let j = 0; j < scope.length; j++) {
                    console.log(guildData.value.countryCode + '',scope[j].country)
                    if (guildData.value.countryCode + '' == scope[j].country || scope[j].country == 'all') {
                        
                        data.value.localName = scope[j].title
...
<template>
    ...
    <div class="bg">
        ...
        <div v-if="data" class="info_list">
            ...
            <div class="title">{{ data.localName }}</div>
            <div class="list_item">
                ...
                <div class="content">
                    ...
                    <div class="content">
                        ...
                        <span v-if="data.conditionRecords" v-for="(item, index) in data.conditionRecords" :key="index">
                            <span
                                v-if="index > 0">&nbsp;{{ data.conditionOpt == '&&' ? multiLan('And') : multiLan('Or') }}&nbsp;</span>
            ...
            <div class="list_item">
                ...
                <div class="content">
                    ...
                    <div v-if="data.joinType != 0" class="content" v-for="(item, index) in data.joinLimits">
                        ...
                        <span v-if="item.limitType == 4">{{ multiLan('Joint limit Activie anchor portion', item.val) }}({{
            multiLan('New anchor online time tips') }})</span>
            ...
            <div class="view_button"
                v-if="data.recordStatus!=0&&data.recordStatus!=1&&data.recordStatus!=5&&data.recordStatus!=6&&data.recordStatus!=7"
                @click="onViewDetail">{{multiLan('View progress')}}</div>
        </div>
